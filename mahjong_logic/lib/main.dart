...
class _GameScreenState extends State<GameScreen> {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      animation: gc,
      builder: (_, __) {
        final me = gc.state.players[gc.humanIndex];

        return Scaffold(
          appBar: AppBar(title: const Text('Mah Jongg')),
          body: Column(
            children: [
              Text('Current: P${gc.state.currentPlayer}  Phase: ${gc.state.phase}'),
              const Divider(),
              const Text('Your hand:'),
              Wrap(
                children: [
                  for (final t in me.hand)
                    Padding(
                      padding: const EdgeInsets.all(4),
                      child: ElevatedButton(
                        onPressed: () => gc.userDiscard(t),
...
/*
import 'package:flutter/material.dart';
import 'logic/mahjong_engine.dart';
import 'ui/portrait-layout.dart';

//void main() => runApp(const MyApp());
void main() {
  final p0 = PlayerState(hand: [Tile(Suit.bam, 2), Tile(Suit.bam, 3)]);
  final p1 = PlayerState(hand: [Tile(Suit.dot, 5), Tile(Suit.dot, 5), Tile(Suit.dot, 5)]);
  final p2 = PlayerState(hand: [Tile(Suit.dot, 5), Tile(Suit.dot, 5), Tile(Suit.bam, 9)]);
  final p3 = PlayerState(hand: [Tile(Suit.crak, 7)]);

  final state = GameState(players: [p0, p1, p2, p3]);
  state.currentPlayer = 1;

  final engine = MahjongEngine(state);

  // Player 1 discards a Dot-5
  engine.dispatch(DiscardAction(1, Tile(Suit.dot, 5)));

  // Now claim window is open; Player 2 can claim pung (has 2 Dot-5s)
  if (engine.canClaimPung(2)) {
    engine.dispatch(ClaimPungAction(2));
    engine.beginPostClaimDiscardPhase(); // claimer's discard phase begins
  }
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(
      ...
      home: PortraitMahjongTableScreen(),//TableEntry(),
    );
  }
}
