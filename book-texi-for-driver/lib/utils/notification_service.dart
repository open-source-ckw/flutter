...
class NotificationService {
  ...
  Future<void> setupInteractedMessage() async {
    ...
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) async {
      ...
      if (message.notification != null) {
        ...
        }else if(message.data['type'] == "Send_Bid_request"){
          Get.to(const OrderMapScreen(),
              arguments: {"orderModel": message.data['orderId']});
        }
        ...
        } else if (message.data['type'] == "intercity_order_payment_complete" || message.data['type'] == "intercity_order_payment_complete") {
  ...
  static Future<String?> checkAndRegenerateToken() async {
    try {
      // Get the current FCM token
      String? token = await FirebaseMessaging.instance.getToken();

      if (token == null || token.isEmpty) {
        // If token is null or empty, it's considered invalid or expired
        print("FCM Token is invalid or expired. Regenerating...");

        // Delete the old token and regenerate a new one
        ...
        if (newToken != null && newToken.isNotEmpty) {
          ...
          throw Exception("Failed to regenerate FCM token.");
        }
      } else {
        // If token is valid, return the current token
        print("FCM Token is valid: $token");
        return token;
      }
    } catch (e) {
      print("Error checking and regenerating FCM token: $e");
      return null;
    }
  }
  ...
  void display(RemoteMessage message) async {
    ...
    try {
      ...
      AndroidNotificationChannel channel = const AndroidNotificationChannel(
        ...
        'wayFinder-driver',
        // 'goRide-driver',
        description: 'Show WayFinder - Driver Notification',
